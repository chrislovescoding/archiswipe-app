...
// Remove direct supabase import if not used elsewhere on this page
// import { supabase } from '@/app/lib/supabaseClient';
import { useAuth } from '@/app/context/AuthContext'; // <--- 1. Import useAuth

export default function Home() {
  const router = useRouter();
  // Remove email state if only used for a removed feature
  // const [email, set_email] = useState('');
  const { session, isLoading: isLoadingAuth } = useAuth(); // <--- 2. Get auth state

  // Remove handler if email input is removed
      ...
      <section className="relative pt-16 pb-32 text-center px-4 z-10">
        ...
        <div className="flex justify-between items-center max-w-6xl mx-auto mb-16">
          ...
          <nav className="hidden md:flex space-x-3 items-center">
            ...
            {isLoadingAuth ? (
              // Optional: Placeholder while loading auth state to prevent layout shift
              <div className="px-4 py-2 h-9 w-20 bg-gray-200 rounded-full animate-pulse"></div>
            ) : session ? (
              // User is logged IN - Show Profile button
              <Link href="/profile" className="px-4 py-2 bg-pink-100 text-pink-700 border border-pink-200 rounded-full hover:bg-pink-200 font-medium shadow-sm transition duration-300">
                Profile
              </Link>
            ) : (
